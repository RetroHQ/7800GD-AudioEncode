...
uint8_t ADPCMEncodeStereo::EncodeChannel(int32_t& index, int32_t& valpred, int16_t val)
{
...
void ADPCMEncodeStereo::EncodeBlock(uint8_t *pADPCMOut, const int16_t *pPCMIn, uint32_t nSamples)
{
	...
	{
		uint8_t nEncoded;
		nEncoded = EncodeChannel(mnIndex[0], mnSample[0], *pPCMIn++) << 4;
		nEncoded |= EncodeChannel(mnIndex[1], mnSample[1], *pPCMIn++);

		*pADPCMOut++ = nEncoded;
	}
}

uint8_t ADPCMEncodeStereo::EncodeSample(const int16_t nLeft, const int16_t nRight)
{
	uint8_t nEncoded;
	nEncoded = EncodeChannel(mnIndex[0], mnSample[0], nLeft) << 4;
	nEncoded |= EncodeChannel(mnIndex[1], mnSample[1], nRight);
	return nEncoded;
}
